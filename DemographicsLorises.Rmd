...
births <- filter(studbook, TypeEvent == "Birth" | TypeEvent == "Capture") %>%
  ...
  distinct() %>% left_join(select(studbook, -c(Sex, Sire, Dam, Date, Location, OrderEvent, DateBirth)), 
...
breeding <- select(births,
  ...
  left_join(select(studbook, -c(Sex, Date, TypeEvent, OrderEvent)), 
                           by = join_by(ID, Location)) %>% distinct() %>%
  group_by(ID) %>% fill(DateBirth, .direction = "downup") %>% ungroup()
```
...
studbook.v2 <- filter(studbook, TypeEvent == "Transfer") %>%
  bind_rows(births) %>% arrange(ID, TypeEvent, Date) %>%
  bind_rows(deaths) %>% arrange(ID, TypeEvent, Date) %>%
  mutate(TypeEvent = fct(TypeEvent, levels = c("Birth", "Transfer", "LTF", "Death"))) %>%
  arrange(ID, TypeEvent, OrderEvent, Date) %>%
  group_by(ID) %>%
  fill(DateBirth, DateDeath, .direction = "downup")

transfers <- filter(studbook.v2, TypeEvent == "Transfer") %>%
  select(ID, Date, To = Location, TypeEvent) %>%
  distinct() %>%
  group_by(ID) %>%
  mutate(Order = row_number() + 1) %>% ungroup()

repro.events <- breeding %>%
  select(ID, Date, To = Location, TypeEvent) %>%
  mutate(From = To) %>% distinct()
...
records <- births %>%
  ...
  mutate(TypeEvent = if_else(TypeEvent %in% c("Death", "LTF"), "End", TypeEvent)) %>%
  ungroup()
```
...
studbook.working <- records %>%
  ...
  group_by(ID) %>%
  mutate(LocLast = if_else(is.na(LocLast) & DateEvent == max(DateEvent), LocEvent, LocLast)) %>%
  fill(LocLast, .direction = "up") %>%
  mutate(DateLast = if_else(is.na(DateLast), today(), DateLast)) %>%
  ...
  left_join(select(breeding,
                   ...
                   Date,
                   Location,
                   TypeEvent), by = join_by(ID, DateEvent == Date, LocEvent == Location, TypeEvent)) %>%
  left_join(select(location.key, 
                   IconBirth  = iconLoc, 
                   ColorBirth = colorLoc, 
                   LocBirth   = Abbrev), 
