---
title: "Inventory Samples and Rxn Products"
...
compilation <- read.csv(path$compilation_csv, header = T)      %>% 
  ...
  mutate(sampleset = paste0(params$sampleset))
...
compilation.export <- compilation %>%
  ...
  mutate(LibraryCode = if_else(sampleset == "loris", 
                               str_replace_all(LibraryCode, "PL0", "hdz"),
                               LibraryCode)) %>%
  mutate(LibraryCode = if_else(sampleset == "loris", 
                               str_replace_all(LibraryCode, "hdz0", "hdz"),
                               LibraryCode)) %>%
  select(SampleID,
         CollectionDate,
         Subj_Certainty,
