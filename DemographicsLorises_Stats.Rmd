...
age.timeline <- assemble_timeline(final.studbook) %>%
  ...
  mutate(OrderEvent = row_number(),
         AgeEvent   = calculate_age(DateBirth, Date)) %>%
  ungroup() %>%
...
```{r}
ages.births.deaths <- age.timeline %>%
  group_by(ID) %>%
...
```{r}
counts.births.deaths <- ages.births.deaths %>%
  group_by(AgeEvent, Month, Event) %>%
  summarize(Count = n()) %>%
  ungroup() %>%
  pivot_wider(id_cols     = c("AgeEvent", "Month"),
              names_from  = "Event",
              values_from = "Count",
              values_fill = 0) %>%
  select(Month, 
  ...
  arrange(Month, AgeEvent)
```


```{r}
monthlyCensus <- census(monthly.presence, "month")
```
...
```{r}
annual_Total <- census(monthly.presence, "year") %>% 
  count_census() %>%
...
```{r}
monthly_Total <- census(monthly.presence, "month") %>% 
  count_census() %>%
