...
```{r}
dem_vars <- demog_variables()
```


```{r}
projections <- proj_matrix(studbook, list(span = 5))
```
...
```{r}
projections_ts <- projections %>% proj_ts()
```

```{r}
projections_test <- projections_studbook(studbook, list(span = 5))
```
