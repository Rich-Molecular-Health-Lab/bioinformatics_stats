...
```{r}
projections <- projections_studbook(studbook, list(span = 5))
```

```{r}
extract_proj <- projections %>%
  mutate(Mx_age5 = map_dbl(Mx_ts, \(x) pluck(x, 5))) 
```

```{r}
extract_proj <- projections %>%
  mutate(Mx_proj5_age5 = map_dbl(Mx_grid, \(x) x[6][5]))
```

```{r}
extract_mx_gd <- projections$Mx_grid
extract_mx_ts <- projections$Mx_ts
```

```{r}
extract_qx_gd <- projections$Qx_grid
extract_qx_ts <- projections$Qx_ts
```

```{r}
Projection_summary <- projections$Projection
```
...
```{r}
year_born <- 2012
life_data <- filter(vitals, cohort_start <= year_born & cohort_end >= year_born)
```
...
lt <- life.table(
  ...
  bind_cols(select(life_data, starts_with("cohort")))
```


```{r}
tables <- make_cohort_lifetables(vitals) %>%
  hoist(lifetable, 
        nMx = "nMx",
        nqx = "nqx",
        lx  = "lx",
        ...
        ex  = "ex") %>%
  select(-lifetable) %>%
  mutate(across(c(nMx:ex), ~map(., \(x) replace_na(x, 0))))

leslies <- make_cohort_leslies(tables)
```



```{r}
cohorts <- cohort_template(studbook)
```

```{r}
studbook_dates <- cohort_studbook(studbook)
```

```{r}
births <- births_annual(studbook)
```
