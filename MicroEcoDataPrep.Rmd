...
```{r}
abund.file.paths <- map(seqruns, \(x) paste0("abund_wf16s_files$", x)) %>%
  set_names(seqruns)
```


```{r}
abundance <- imap(abund.file.paths, ~ {
  path   <- .x
  seqrun <- .y
  
  df <-   read.table(paste0(path),
                     sep    = "\t",
                     header = TRUE)
  
  df.list <- list(seqrun = df)
...
```{r}
abundance.files.list <- list.files(
                              path       = path$abundance_wf16s,
                              pattern    = "*_abundance_table_species.tsv$", 
                              full.names = TRUE) %>%
                         as.list()
```
...
```{r}
abundance.filenames <- list.files(
                              path       = path$abundance_wf16s,
                              pattern    = "*_abundance_table_species.tsv$", 
                              full.names = FALSE) %>%
                       as.list()
...
```{r}
seqrun               <- str_remove(abundance.filenames, "_abundance_table_species.tsv")

abundance.files      <- lapply(abundance.files.list, read_abundance_file) %>%
  set_names(seqrun)

abundance            <- imap(abundance.files, ~ {
  seqrun <- .y
  df     <- .x
  
  df %>% rename_with(~gsub(".", "-", .x, fixed = TRUE))
...
abundance.files <- list.files(path       = path$abundance_wf16s,
                              pattern    = "*_abundance_table_species.tsv$", 
                              full.names = TRUE)
abundance.names <- list.files(path       = path$abundance_wf16s,
                              pattern    = "*_abundance_table_species.tsv$", 
                              full.names = FALSE)

abundance <- tibble()
seqrun <- str_remove(abundance.names, "_abundance_table_species.tsv")
