...
dataset_tbl <- sample_table %>%
  ...
  select(all_of(categ_cols_env), study_day, bristol_mean) %>%
  mutate(rowname = ID) %>%
...
```{r}
bubble_numeric_vars <- numeric_vars %>%
  select(where(~min(.x) != max(.x))) %>%
  pivot_longer(where(is.numeric)) %>%
  ggplot( aes(x = name, y = value, color = name)) +
    geom_count(alpha = 0.7) +
    geom_point(aes(size = after_stat(n)), 
...
plot_bubble <- ggplotly(bubble_numeric_vars)
