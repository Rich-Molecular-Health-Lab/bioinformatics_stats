...
```{r}
start <- min(as_date(data_genus$sample_table$collection_date))
end   <- ceiling_date(max(as_date(data_genus$sample_table$collection_date)), "month")
...
months <- map(seq(start, end, by = "month"), \(x) interval(floor_date(x, "months"), (ceiling_date(x, "months") - days(1)))) %>%
  ...
  select(-interval)
weeks  <- map(seq(start, end, by = "week"), \(x) interval(floor_date(x, "weeks"), (ceiling_date(x, "weeks") - days(1)))) %>%
...
summary <- sample_table %>%
  rownames_to_column("SampleID") %>%
  right_join(weeks, by = join_by(collection_date, collection_month, collection_week))

sample_timeline <- plot_ly(sample_table) %>%
  add_histogram(
    x        = ~collection_date , 
    histfunc = "count",
    nbinsx   = max(sample_table$collection_month)
    ) %>%
  add_histogram(
    x        = ~collection_date , 
    histfunc = "count",
    nbinsx   = max(sample_table$collection_week),
    xaxis    = "x2",
    yaxis    = "y2"
    ) %>%
  layout(
