...
metadata <- samples %>% 
  ...
  distinct() %>%
  arrange(study_day, subject, SequenceID)
```
...
wide_subtables <- function(tbl, class) {
  tbl %>% select(SampleID, ExtractID, SequenceID, class) %>% 
  unnest(class) %>% 
  select(SampleID, ExtractID, SequenceID, nutrient, fed) %>%
  pivot_wider(id_cols     = c("SampleID", "ExtractID", "SequenceID"), 
              names_from  = "nutrient", 
              values_from = "fed")
}
```
...
CHOs_wide     <- wide_subtables(metadata, "CHOs")
Ash_wide      <- wide_subtables(metadata, "Ash")
vitamins_wide <- wide_subtables(metadata, "vitamins")
subtables <- proteins_wide %>% 
  left_join(fats_wide, by = join_by(SampleID, ExtractID, SequenceID)) %>% 
  left_join(CHOs_wide, by = join_by(SampleID, ExtractID, SequenceID)) %>% 
  left_join(Ash_wide, by = join_by(SampleID, ExtractID, SequenceID)) %>% 
  left_join(vitamins_wide, by = join_by(SampleID, ExtractID, SequenceID))
```
...
reactable(
              ...
              height              = 600,
              columns             = list(
                proteins = colDef(cell = function(value, index) {
    ...
    htmltools::div(
          style = "margin-top: 5px; margin-left: 20px; font-size: 12px;",
      reactable(proteins_tbl,
                fullWidth = F
                  )
                )
                })))
