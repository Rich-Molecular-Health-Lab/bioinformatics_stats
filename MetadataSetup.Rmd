...
vis_metadata <- metadata_summary %>% 
  ...
  total_mg        = colDef(header = tippy_total_mg(),
                           ...
                           details = function(index) {food_subtab(index)}),
  protein_fed     = colDef(header = tippy_protein(),
                           cell = nutrient_cell(metadata_summary),
                           details = function(index) {nutrient_details(index, proteins_expand)}),
  fat_fed         = colDef(header = tippy_fat(),
                           cell = nutrient_cell(metadata_summary),
                           details = function(index) {nutrient_details(index, fats_expand)}),
  CHO_fed         = colDef(header = tippy_chos(),
                           cell = nutrient_cell(metadata_summary),
                           details = function(index) {nutrient_details(index, CHOs_expand)}),
  mineral_fed     = colDef(header = tippy_ash(),
                           cell = nutrient_cell(metadata_summary),
                           details = function(index) {nutrient_details(index, Ash_expand)}),
  vitamins        = colDef(header = tippy_vitamins(),
                           cell = function(value) {"Expand Details \u230e"},
                           details = function(index) {nutrient_details(index, vitamins_expand)}),
