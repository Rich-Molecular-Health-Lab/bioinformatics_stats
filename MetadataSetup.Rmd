...
studbook.info <- read.csv(path$metadata$studbook, header = TRUE) %>%
  mutate(
          Subject    = factor(str_to_lower(Name), levels = unique()),
          StudbookID = factor(ID, levels = unique()),
          MotherID = factor(Mother, levels = unique()),
          FatherID = factor(Father, levels = unique()),
          FatherID = factor(Father, levels = unique())
          )
  select(
    Subject       = Name,
