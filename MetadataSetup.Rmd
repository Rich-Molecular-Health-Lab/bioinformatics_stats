...
samples <- read.table(path$compilation, header = T, sep = "\t") %>%
  mutate(SeqDate        = ymd(SeqDate),
         ...
         mutate(collection_y      = collection,
...
setkey(samples.culi.dt,   collection, collection_y)
setkey(samples.warb.dt,   collection, collection_y)
```
...
samples.diet  <- foverlaps(samples.dt, diet.dt, type = "any", nomatch = NA, mult = "all") %>% tibble() %>%
  select(-c("collection", 
            ...
            "start",
            "end")) %>%
  mutate(subj_diet_trial = diet_trial, .keep = "unused") %>%
  left_join(samples.culi.diet, by = join_by(SampleID))
```
...
```{r message=FALSE, warning=FALSE}
notes.repro.dt <- notes.repro %>%
         mutate(start        = decimal_date(as.POSIXct(start))) %>%
         mutate(end          = start) %>%
        as.data.table()
setkey(notes.repro.dt, start, end)  

samples.notes.repro <- foverlaps(samples.nosubj.dt, notes.repro.dt, type = "any", nomatch = 0L) %>%
  tibble() %>%
  select(-c("collection", 
            "collection_y",
            "start",
...
```{r message=FALSE, warning=FALSE}
samples.repro <- samples.notes.repro %>%
                 full_join(samples.estrus, by = join_by(SampleID)) %>%
                 full_join(samples.access, by = join_by(SampleID)) %>%
  arrange(SampleID)
```
...
samples.metadata <- samples.set1.2 %>% left_join(bristol.range,
  ...
  relocate(SampleID, ExtractID, SequenceID)
```


# Final Version of Dataframe

Now, I am going to join back in some sample inventory tables. Right now the data are organized with one row per SampleID, but I am going to expand that so each SampleID row gets split into daughter rows for each ExtractID and SequenceID. This will make it easier to manipulate metadata later based on specific sequencing runs, as we will have multiple runs for the same sample sometimes.  

```{r}
export.metadata <- samples.metadata %>%
