...
page_fluid(
    accordion(
      ...
      accordion_panel(
        ...
        tagList(tags$pre(includeText(here("metadata/loris/loris_nutrition.R"))))
    ),
      accordion_panel(
...
```{r}
nutr_protein <- enframe(proteins, name = "nutrient") %>%
  mutate(nutrient_class = "proteins",
         nutrient = str_replace_all(nutrient, "totals", "total")) %>%
  unnest_longer(value, indices_to = "diet_name")  %>%
  unnest_wider(value) %>%
  mutate(relative_unit = "proportion",
         relative_fed  = proportion, .keep = "unused")

nutr_fat <-   enframe(fats, name = "nutrient") %>%
  mutate(nutrient_class = "fats",
         nutrient = str_replace_all(nutrient, "totals", "total")) %>%
  unnest_longer(value, indices_to = "diet_name")  %>%
  unnest_wider(value) %>%
  pivot_longer(cols      = c("proportion", "mg_g"),
               names_to  = "relative_unit",
               values_to = "relative_fed",
               values_drop_na = T)

nutr_CHOs <-   enframe(CHOs, name = "nutrient")  %>%
  mutate(nutrient_class = "CHOs",
         nutrient = str_replace_all(nutrient, "totals", "total")) %>%
  unnest_longer(value, indices_to = "diet_name")  %>%
  unnest_wider(value) %>%
  mutate(relative_unit = "proportion",
...
nutr_Ash <-  enframe(Ash, name = "nutrient")   %>%
  mutate(nutrient_class = "Ash",
         nutrient = str_replace_all(nutrient, "totals", "total")) %>%
  unnest_longer(value, indices_to = "diet_name")  %>%
  unnest_wider(value) %>%
  pivot_longer(cols      = c("proportion", "mg_kg"),
               names_to  = "relative_unit",
               values_to = "relative_fed",
               values_drop_na = T) %>% select(-ca_p_ratio)

nutr_vitamins <- enframe(vitamins, name = "nutrient")    %>%
  mutate(nutrient_class = "vitamins",
         nutrient = str_replace_all(nutrient, "totals", "total")) %>%
  unnest_longer(value, indices_to = "diet_name")  %>%
  unnest_wider(value) %>%
  pivot_longer(cols      = c("mg_kg", "IU_g", "ug_kg"),
               names_to  = "relative_unit",
               values_to = "relative_fed",
               values_drop_na = T)
```
...
diet_totals <- enframe(nutrient_totals, name = "diet_name") %>%
  unnest_wider(value) %>%
  mutate(nutrient = "total", nutrient_class = "total") %>%
