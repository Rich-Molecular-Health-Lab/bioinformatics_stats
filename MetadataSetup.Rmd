...
diet_supplements_daily <- enframe(supplements, name = NULL) %>%
  ...
  select(-c(begin, end)) %>%
  relocate(date) %>%
  right_join(diet_daily, by = join_by(subject, date))
```
...
metadata_daily <- tibble(date = seq.Date(from = ymd(path$day1), 
  ...
  left_join(diet_supplements_daily, by = join_by(subject, date))
```
...
It is a good idea to exporta  backup file with this complete table, but first I need to unnest those nested columns. This makes the table very large, so I will export a version without those nested columns and then export those individual tables as their own spreadsheets in the external script.  
  
I am also going to use this version for a visual summary table that we can look at to brainstorm variables. I will replace the nested columns with a summary value for each, and later I can add those subtables back in as an expandable row using reactable.

```{r}
metadata_summary <- metadata %>% 
  hoist("foods"   , foods          = list("food"), .transform = ~ str_c(., sep = ", ")) %>% 
  hoist("fats"    , fat_total      = list("fed", -1)) %>% 
  hoist("proteins", protein_total  = list("fed", -1)) %>% 
  hoist("CHOs"    , CHOs_total     = list("fed", -1)) %>% 
  hoist("Ash"     , protein_total  = list("fed", -1)) %>% 
  hoist("vitamins", vitamins_total = list("fed", -1)) %>%
  select(-c(proteins,
            fats    ,
            CHOs    ,
            Ash     ,
...
```{r}
write.table(metadata_summary, here(path$metadata$summary), row.names = F, sep = "\t")
```

```{r}
metadata_summary <-  reactable(
              metadata,
              resizable           = T,
              filterable          = T,
