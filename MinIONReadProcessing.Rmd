...
params:
  taxon: "loris"
  parent_dir: "/work/richlab/aliciarich/ont_reads/"
  working_dir: "/work/richlab/aliciarich/ont_reads/microbiomes_loris"
  seqrun: "hdz19"
...
#!/bin/bash
#SBATCH --time=2:00:00
#SBATCH --job-name=params$seqrunbasecall
#SBATCH --error=params$working_dir/logs/params$seqrunbasecall_%A_%a.err
#SBATCH --output=params$working_dir/logs/params$seqrunbasecall_%A_%a.out
...
```{terminal}
srun --partition=gpu,guest_gpu --time=2:30:00 --nodes=1 --ntasks=1 --cpus-per-task=8 --mem=256GB --gres=gpu:1 --job-name=params$seqrunbasecall --pty $SHELL
```
...
pod5="params$working_dir/hdz_raw/params$seqrun/"
basecalled="params$working_dir/basecalled/params$seqrun/"
mkdir -p $basecalled

algo="params$parent_dirmethods_16s$dorado_model"
...
#SBATCH --job-name=params$seqrundemux_trim
#SBATCH --error=params$working_dir/logs/params$seqrundemux_trim_%A_%a.err
#SBATCH --output=params$working_dir/logs/params$seqrundemux_trim_%A_%a.out
...
basecalled="params$working_dir/basecalled/params$seqrun/"
sample_sheet="params$working_dir/dataframes/params$seqrun_sample_sheet.csv"
trimmed="params$working_dir/trimmed/params$seqrun"
...
#!/bin/bash
#SBATCH --time=00:45:00
#SBATCH --job-name=params$seqrun_qc
#SBATCH --error=params$working_dir/logs/params$seqrun_qc_%A_%a.err
#SBATCH --output=params$working_dir/logs/params$seqrun_qc_%A_%a.out
#SBATCH --partition=guest
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --mem=100GB
...
```{terminal}
srun --partition=guest --nodes=1 --ntasks-per-node=1 --job-name=params$seqrun_qc --mem=100GB --time=0:45:00 --pty $SHELL
```
...
trimmed="params$working_dir/trimmed/params$seqrun"
filtered="params$working_dir/filtered/params$seqrun"
...
#SBATCH --job-name=params$seqrun_wf16s
#SBATCH --error=params$working_dir/logs/params$seqrun_wf16s_%A_%a.err
#SBATCH --output=params$working_dir/logs/params$seqrun_wf16s_%A_%a.out
#SBATCH --partition=guest
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
```{terminal}
srun --partition=guest --nodes=1 --ntasks-per-node=1 --job-name=params$seqrun_wf16s --mem=350GB --time=03:00:00 --cpus-per-task=32 --pty $SHELL
```
...
filtered="params$working_dir/filtered/params$seqrun/"
processed="params$working_dir/data/outputs_wf16s/params$seqrun"
