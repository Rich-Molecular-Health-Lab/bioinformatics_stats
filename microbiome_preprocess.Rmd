...
skim_phyloseq <- function(phyloseq) {
 psmelt(phyloseq) %>% 
    select(-c(
      ...
      subject_age)) %>%
    group_by(subject) %>% my_skim() %>% select(-complete_rate) %>% mutate(across(where(is.numeric), ~round(., 2)))
}

view_tree <- function(phyloseq) {
  ggtree(phyloseq, branch.length = "none", aes(color = Phylum)) + 
  scale_color_ucscgb("default", na.value = "#00000080") +
...
view_tree(cla_phylo) + ggtitle("Class-Level OTUs")  + geom_text(aes(label = Class ), hjust = -0.05, size = 2) + coord_cartesian(clip = "off") + theme(plot.margin = margin(5, 65, 5, 5, "pt"))
view_tree(phy_phylo) + ggtitle("Phylum-Level OTUs") + geom_text(aes(label = Phylum), hjust = -0.05, size = 3) + coord_cartesian(clip = "off") + theme(plot.margin = margin(5, 65, 5, 5, "pt"))
```
