...
data_working <- esr_data %>%
  ...
  group_split() %>%
  set_names(., map(., \(x) paste0(first(x$groups_treatment)))) %>%
...
save_html(scatter_navset, "visuals/receptor_assays/dose_response_raw.html")
```
...
## Split Data into Nested Lists

### By Species + Receptor + Plate + Treatment

```{r}
data_plate <- data_working %>%
  select(
    group = group_plate,
    dose,
    response
  ) %>%
  filter(n_distinct(dose) > 2, .by = "group") %>%
  group_by(group) %>%
  group_split() %>%
...
```{r}
plates_drc <- keep_at(data_plate, names(ll4_plates$results)) %>%
  map(\(x) drm(
    formula = response ~ dose, 
    data    = x, 
    fct     = LL.4(
...
pooled_drc <- keep_at(data_pooled, names(ll4_pooled$results)) %>%
  map(\(x) drm(
    formula = response ~ dose, 
    data    = x, 
    fct     = LL.4(
...
```{r}
plates_coefs_tbl <- map(plates_working, \(x) coef(x)) %>%
  enframe(name = "group_plate", value = "coefs") %>%
  unnest_wider(coefs) %>%
  rename_with(~str_remove_all(., ":(.+)")) %>%
  left_join(meta_working, by = "group_plate") %>%
  select(-color_key) %>%
  col_list("groups_treatment") %>%
    arrange(receptor, dose_type, groups_treatment, treatment)
```


```{r}
plates_coefs <- plates_coefs_tbl %>%
    group_by(labs_er) %>%
    group_split() %>%
...
```{r}
fitted <- map(ll4_plates, \(x) augment(x, x[["origData"]], conf.int = TRUE)) %>%
  map(\(x) map_qqnorm(x)) %>%
   list_rbind() %>%
  rename(group_plate = group) %>%
  rename_with(~str_remove_all(., "\\.")) %>%
  left_join(meta_working, by = "group_plate") %>%
...
resid_qq_complete <- navset_pill_list(!!!resid_panels,
                                      widths = c(2, 10))
save_html(resid_qq_complete, "visuals/receptor_assays/residuals.html")
```


### Dose-Response Curves (Predicted and Fitted)

```{r}
predicted <- map(
  ll4_plates, \(x) predict(
    x, data.frame(
      dose = seq(
        min(x[["dataList"]][["dose"]]), 
        max(x[["dataList"]][["dose"]]), 
  ...
  map2(
   ...
   list_rbind() %>%
  rename(fold_act       = Prediction,
         fold_act_lower = Lower,
         fold_act_upper = Upper) %>%
  left_join(meta_working, by = "group_plate") %>%
...
```{r}
predicted_list <- dose_response_list(predicted)
```



```{r}
curve_panels <- imap(predicted_list, \(x, idx) dose_response_grid(x, idx, "plot_curve")) %>%
  set_names(., "")

curve_complete <- navset_pill_list(!!!curve_panels,
                                      widths = c(2, 10))
save_html(curve_complete, "visuals/receptor_assays/dose_response_curves.html")
```
...
```{r}
raw_data_list <- data_working %>%
  left_join(meta_working, by = "group_plate") %>%
  dose_response_list()
```

```{r}
raw_panels <- imap(raw_data_list, \(x, idx) dose_response_grid(x, idx, "plot_raw")) %>%
  set_names(., "")

raw_scatter_complete <- navset_pill_list(!!!raw_panels, widths = c(2, 10))
save_html(raw_scatter_complete, "visuals/receptor_assays/dose_response_raw.html")
```
...
dashboard <- navset_tab(
  nav_panel("Model Selection"     , msel_plots),
  nav_panel("Dmax Boxplots"       , plates_dmax),
  nav_panel("Residuals"           , resid_qq_complete),
  nav_panel("Dose-Response Curves", curve_complete),
  nav_panel("Raw Dose-Response"   , raw_scatter_complete)
)

save_html(dashboard, "visuals/receptor_assays/model_selection_dashboard.html")
```

### Dashboard Notes

**Exclude the following Treatments:**
