...
treatments <- c(
   ...
   estrogen_estradiol    =  "E2"            ,
   estrogen_estrone      =  "E1"            ,
   estrogen_estriol      =  "E3"            ,
   metabolite_genistein  =  "GEN"           ,
   metabolite_daidzein   =  "DAID"          ,
   metabolite_coumestrol =  "COU"           ,   
   metabolite_equol      =  "EQUOL"         ,
   metabolite_enterodiol =  "ENTDIOL"       ,
...
tick_text <- function(data) {
  as.list(pull(data, ticklab_y))
}

tick_vals <- function(data) {
  as.list(pull(data, dummy_y))
}
...
db_ercols <- function(list_receptors) {
...
edx_tbl <- edx %>%
  ...
  mutate(ticklab_y = if_else(subject == "loris", labs_treatment, "")) %>%
  nest(.by = c("groups_treatment", "labs_er")) %>%
  mutate(shapes   = map(data, \(x) db_shapes(x))) %>%
  mutate(yaxis    = map(data, \(x) db_yaxis(x))) %>%
  mutate(xaxis    = map(data, \(x) db_xaxis(x))) %>%
  nest(.by = c("groups_treatment", "labs_er")) %>%
...
```{r}
edx_dbplot <- map(edx_tbl, \(x) db_layout_receptors(x))
